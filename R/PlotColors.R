...
PlotColors<-function(color_list,color_list_name=NULL, n=NULL){
  ...
  p<-ggplot(data_table,aes(x=colors))+geom_bar(aes(fill=colors))+theme_tufte()+labs(x=paste(unique(color_list), collapse=', '))+
    theme(
