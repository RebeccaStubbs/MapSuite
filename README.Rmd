...
```{r global_options, include=FALSE,dev='svg'}
knitr::opts_chunk$set(fig.pos = 'h')
knitr::opts_chunk$set(fig.width = 10)
knitr::opts_chunk$set(dpi=1000, fig.height = 3)
```

```{r,echo=F,eval=TRUE,message=F, include=F,dev='svg'}
library(MapSuite)
library(xtable)
library(classInt)
...
The MapSuite package uses the **ggplot2** library as a launching point for making maps, using three main functions (PolygonMap, PointMap, and RasterMap) as a wrapper for **ggplot2**'s geom_polygon, geom_point and geom_raster geometries. Unlike building a plot from scratch in **ggplot2**, these functions demand relatively few inputs to create a pleasing choropleth map. The default settings of the MapSuite are designed to follow a set of graphical guidelines set forth by E.R. Tufte[14].
...
```
```{r, echo=F, dpi=1000, fig.height=3, fig.width=10, fig.align='center', message=F, warning=F,fig.cap="Mapping Point, Polygon, and Raster Data",dev='svg'}
...
```
```{r, echo=F, dpi=1000, fig.height=3, fig.width=10, fig.align='center', message=F, warning=F,fig.cap="Changing basic map aesthetics and adding outline polygons",dev='svg'}
...
```{r,dpi=1000, fig.height=3, fig.width=10,fig.align='center', message=F, warning=F,fig.cap="Fig 3: Using outlines to add emphasis",dev='svg'}
...
```
```{r, echo=F, results='hide', eval=T,dev='svg'}
county_polygons@data[,above_below:=ifelse(county_polygons@data[["elevation"]]>1000, "<1,000 ft",">1,000 ft")]
```
```{r, echo=F, dpi=1000, fig.height=3, fig.width=10,fig.align='center', message=F, warning=F,fig.cap="Mapping binned and categorical data",dev='svg'}
continuous<-PolygonMap(map=county_polygons, id="cnty", variable="elevation", map_title="Mean Elevation", return_objects = T, font_size=12)
categorical<-PolygonMap(map=county_polygons, id="cnty", variable="above_below", legend_title = "Mean Elevation",return_objects = T, font_size=12)
...
```{r,dpi=1000, fig.height=3, fig.width=10,fig.align='center', message=F, warning=F,fig.cap="Changing color schemes and legend properties",dev='svg'}
...
```{r,dpi=1000, fig.height=3, fig.width=10,fig.align='center', message=F, warning=F,fig.cap="Modifying point size based on another variable",dev='svg'}
...
```{r, echo=F, fig.align='center', fig.width=10, fig.height=8, message=F, warning=F,dev='svg'}
library(imager)
im <- load.image("C:/Users/stubbsrw/Documents/us_counties_stubbs_gitrepo/MapSuite/scrambled_poly.png")
plot(im, main=NULL, xlab=NULL,ylab=NULL,axes=FALSE)
```
...
```{r, echo=F, dpi=1000, fig.height=3, fig.width=10, fig.align='center', message=F, warning=F, fig.cap="Mapping two different variables from an external data frame",dev='svg'}
var1<-PolygonMap(map=county_polygons,data=county_simulated,font_size=12,
           id="cnty", variable="var1", map_title="Simulated Variable 1", return_objects = T, include_titles = T)$map
var2<-PolygonMap(map=county_polygons,data=county_simulated,font_size=12,
...
```{r, echo=F, dpi=1000, fig.height=2, fig.width=10, fig.align='center', message=F, warning=F,dev='svg'}
seq<-PlotColors(wpal("cool_blue_jeans", 9), color_list_name="Example of Sequential palette",base_size = 12)+theme(axis.text = element_blank())
div<-PlotColors(wpal("diverging_pink_black_blue", 9), color_list_name="Example of Diverging palette", base_size = 12)+theme(axis.text = element_blank())
...
```{r, echo=F, dpi=1000, fig.height=3, fig.width=10, fig.align='center', message=F, warning=F,dev='svg'}
library(RColorBrewer)
brew<-PlotColors(brewer.pal(9,"Spectral"), color_list_name="Color Brewer Spectral Palette",base_size = 12)+theme(axis.text = element_blank())
cube<-PlotColors(wpal("ld_reg1", 9), color_list_name="A Cubehelix Color Palette",base_size = 12)+theme(axis.text = element_blank())
cube2<-PlotColors(wpal("ld_bright3", 9), color_list_name="Another Cubehelix Color Palette",base_size = 12)+theme(axis.text = element_blank())
...
```{r, echo=T,dpi=1000, fig.height=1, fig.width=10, fig.align='center', message=F, warning=F,dev='svg'}
...
```{r, echo=F, dpi=1000, fig.height=3, fig.width=10, fig.align='center', message=F, warning=F, fig.cap="The same data, represented using three different color schemes",dev='svg'}
grid.newpage() # Starting a new page
...
```{r, echo=F, dpi=1000, fig.height=3, fig.width=10, fig.align='center', message=F, warning=F, fig.cap="Two different binning methods",dev='svg'}
...
```{r, echo=F, dpi=1000, fig.height=6, fig.width=10, fig.align='center', message=F, warning=F, fig.cap="Changing how the colors are stretched across the data: (i) Evenly from the minimum to the maximum (top left), (ii) Artificially extending the range of the colors to 0-300, which is larger than the range of the data (top right), and (iii) Changing whether light or dark colors occupy more of the range of the data by shifting how the same colors are applied to the dataas in top left, or in a way where more dark or light colors cover the majority of the range (bottom)",dev='svg'}
default<-PolygonMap(map=colorado, 
                   ...
                   legend_bar_length=unit(.4,"snpc"), font_size=12,
                   map_colors = rev(wpal("parrotfish")),
                   legend_position = "left",
                   return_objects = T,
                   include_titles = T)$map
...
```{r, echo=F, dpi=1000, fig.height=3, fig.width=10, fig.align='center', message=F, warning=F, fig.cap="Using two different numeric values (20 and 50) to center the color scheme on a meaingful value",dev='svg'}
...
```{r,dpi=1000, fig.height=3, fig.width=10, fig.align='center', message=F, warning=F,dev='svg'}
...
```{r,dpi=1000, fig.height=4, fig.width=4, fig.align='center', message=F, warning=F, fig.cap="A map with a histogram of the data distribution",dev='svg'}
...
results<-PolygonMap(map=colorado, id="cnty", data=county_simulated_time,
            ...
            map_subtitle="Year: ", map_title="Var 3",font_size=12,return_objects = T,include_titles=T)
...
```{r,eval=F,message=F, warning=F,dev='svg'}
...
```{r, echo=T, dpi=1000, fig.height=6, fig.width=10, fig.align='center', message=F, warning=F,dev='svg'}
