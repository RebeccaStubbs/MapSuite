---
title: 'MapSuite: An R Package for Thematic Maps'
...
```{r,echo=F,eval=TRUE,message=F, include=F}
library(MapSuite)
library(xtable)
library(classInt)
...
#save(county_centroids,county_pixels,county_data,county_simulated,county_polygons,file="C:/Users/stubbsrw/Documents/thesis_files/MapSuite/data.rdata")
# census_regions<-fread("C:/Users/stubbsrw/Documents/thesis_files/MapSuite/census_regions.csv")
# county_data<-county_data[year==2014,]
# data<-merge(county_polygons@data,census_regions,by="state_name")[order(cnty)]
# county_polygons@data<-data; rm(data,census_regions)
# 
# setnames(county_data,"mcnty","cnty")
#county_polygons@data<-merge(county_polygons@data,county_data[year==2014,list(elevation=elev_mean,cnty)],by="cnty")
...
The ability for scientists, students, and citizens to easily visualize and analyze spatial data is more crucial than ever. As the amount of freely available geo-referenced data continues to grow, an ever-increasing community is discovering and utilizing spatial information[1]. A host of commercial and open source software with point-and-click interfaces exist for spatial analysis and visualization-- from ESRI's software suite to QGIS, Google Maps and other online options \citep{overview_dekstop_gis}. For those who wish to move beyond the graphical user interface, open-source languages such as R and Python are no longer restricted to those with academic or computer science backgrounds. Massive online open courses, an extensive blogging community, and online forums such as StackExchange provide instruction and trouble-shooting help from any corner of the world. The barriers to a truly participatory geographic information science (GIS) community are being lowered. This explosion of free resources empowers citizen scientists, grassroots organizations, and institutions in low-resource settings to take advantage of the ever-growing library of open-source toolkits for data science. R, as an open-source programming language used extensively for data analysis and statistics, has a variety of tools designed for spatial analysis and visualization \citep{R} \citep{sp}.

Working with spatial data is not, however, straightforward. Full of idiosyncrasies such as varying data types, projections, and the threat of invalid geometries, there is an additional overhead to working with spatial data. This poses a challenge: how can users with less experience in programming and GIS be brought into the fold? The ability to present and visualize findings from any analysis is critical, and map-making has been shown to be a key tool. Empowered communities have the ability to present maps to decision makers, leading to policy changes \citep{participatory_GIS_overview}. 

The **MapSuite** package joins a community of prior work on mapping in R. In addition to the **sp** package \citep{sp}, which contains both spatial data types and simple plotting functions, a variety of packages focus on spatial data visualization. Finely-customized graphics of many kinds are possible using **ggplot2** \citep{ggplot2}, which offers a number of data-driven plotting functions. Mapping-specific packages, such as **ggmap**
\citep{ggmap}, **ggspatial** \citep{ggspatial}, **rCarto** \citep{rCarto}, and **choroplethR** \citep{choroplethR}, each present ways for R users to visualize spatial data. However, none of the currently existing packages emphasize (i) the ability to map quickly and easily over time or subgroup, or (ii)  the ability to layer complex maps to create a more sophisticated cartographic output. The growing popularity of interactive tools, such as **Leaflet** \citep{leaflet} and **Shiny** \citep{shiny}, have changed the landscape of what it means to visualize multi-dimensional data. However, generating images and PDFs of results is still a key way to share and present results. 

Many of the static graphics packages, such as **ggplot** and **ggmap**, utilize a paradigm established in R by Hadley Wickham called the "grammar of graphics" \citep{grammar_graphics}. Within the "grammar of graphics," a plot is built in layers, where the data forms the foundation of the plot, and different geometric outputs (points, paths/lines, and polygons) can be layered and manipulated to generate a diagram. This approach to code has many advantages-- the syntax is highly flexible and well-suited to the kinds of data used by and produced from statistical analyses. There is also extensive documentation on how to use **ggplot2**, and how to achieve a variety of effects using this flexible package.

The MapSuite package uses the **ggplot2** library as a launching point for making maps, using three main functions (PolygonMap, PointMap, and RasterMap) as a wrapper for **ggplot2**'s geom_polygon, geom_point and geom_raster geometries. Unlike building a plot from scratch in **ggplot2**, these functions demand relatively few inputs to create a pleasing choropleth map. The default settings of the MapSuite are designed to follow a set of graphical guidelines set forth by E.R. Tufte \citep{tufte}.

 

### Maximizing the data-to-ink ratio

Edward Tufte was well-known for the philosophy that effective graphics maximize information transfer while minimizing distractions. According to Tufte, in his book _The Visual Display of Quantitative Information_, the principles of graphical excellence are as follows: 
...
```{r, echo=F,include=T,message=F,results='asis'}
library(data.table)
library(xtable)
items<-data.table(obj=c("county_polygons","county_centroids","county_pixels","county_simulated", "county_simulated_time"),
                  desc=c("A SpatialPolygonsDataFrame of county boundaries","A data.table with coordinates for county centroids", "A data.table with coordinates on a regular grid","A data.table of simulated data  at the county level","A data.table of simulated data at the county level, multiple time periods"))
setnames(items,names(items),c("Object Name","Description"))
print(xtable(items), comment=F, include.rownames=F)
```

Each of the polygons, centroids, and pixels objects contain columns with a unique identifier ('cnty'), and columns for state name ('state_name'), and the mean elevation of the county ('elevation'). The data and spatial objects can be subset by state name, allowing for graphics focusing on a particular area (the states of Washington and Colorado are emphasized in these examples).
...
```
```{r, echo=F, fig.height=1.5, fig.width=6, fig.align='center', message=F, warning=F,fig.cap="Mapping Point, Polygon, and Raster Data"}
poly<-PolygonMap(map=wa_polygons,
           id="cnty",
           return_objects = T)
point<-PointMap(coords=wa_centroids,
         id="cnty", xcol="x_albers",ycol="y_albers",
          return_objects = T)
raster<-RasterMap(coords=wa_pixels,
         id="cnty", xcol="x_albers", ycol="y_albers",
         return_objects = T)

cowplot::plot_grid(poly$map, point$map, raster$map, labels = c("Polygon", "Polygon", "Raster"), label_size = 12, nrow=1, align="h")
```
...
```
```{r, echo=F, fig.height=1, fig.width=4.5, fig.align='center', message=F, warning=F,fig.cap="Changing basic map aesthetics and adding outline polygons"}
poly<-PolygonMap(map=wa_polygons, id="cnty",
         map_colors = "lightgreen", map_outline_color ="black",
         return_objects = T)
point<-PointMap(coords=wa_centroids,id="cnty", xcol="x_albers", ycol="y_albers",
         map_colors = "red", pointsize = 1, font_family="sans",
         outline = wa_polygons, outline_size = 1, outline_color = "black", return_objects = T)
cowplot::plot_grid(poly$map, point$map, nrow=1, align="h")
```
...
```{r,fig.height=1.5, fig.width=4, fig.align='center', message=F, warning=F,fig.cap="Fig 3: Using outlines to add emphasis"}
...
county_polygons@data[,above_below:=ifelse(county_polygons@data[["elevation"]]>1000, "<1,000 ft",">1,000 ft")]
```
```{r, echo=F, fig.height=1.5, fig.width=5, fig.align='center', message=F, warning=F,fig.cap="Mapping binned and categorical data"}
continuous<-PolygonMap(map=county_polygons, id="cnty", variable="elevation", map_title="Mean Elevation", return_objects = T, font_size = 8)
categorical<-PolygonMap(map=county_polygons, id="cnty", variable="above_below", legend_title = "Mean Elevation",return_objects = T, font_size = 8)

grid.newpage() # Starting a new page
pushViewport(viewport(layout = grid.layout(1, 2)))
print(continuous$map, vp = vplayout(1, 1))
print(categorical$map, vp = vplayout(1, 2))
```
...
```{r,fig.height=1.5, fig.width=5, fig.align='center', message=F, warning=F,fig.cap="Changing color schemes and legend properties"}
...
```{r,fig.height=1.5, fig.width=5, fig.align='center', message=F, warning=F,fig.cap="Modifying point size based on another variable"}
...
```{r, echo=F, fig.height=2, fig.width=4, fig.align='center', message=F, warning=F, fig.cap="Mapping two different variables from an external data frame"}
var1<-PolygonMap(map=county_polygons,data=county_simulated,font_size=9,
           id="cnty", variable="var1", map_title="Simulated Variable 1", return_objects = T, include_titles = T)$map
var2<-PolygonMap(map=county_polygons,data=county_simulated,font_size=9,
           id="cnty", variable="var2", map_title="Simulated Variable 2",  return_objects = T, include_titles = T)$map
grid.newpage() # Starting a new page
pushViewport(viewport(layout = grid.layout(1, 2)))
print(var1, vp = vplayout(1, 1))
print(var2, vp = vplayout(1, 2))
```
...
```{r, echo=T, fig.height=.5, fig.width=5, fig.align='center', message=F, warning=F}
...
```{r, echo=F, fig.height=1.5, fig.width=6, fig.align='center', message=F, warning=F, fig.cap="The same data, represented using three different color schemes"}
grid.newpage() # Starting a new page

calm<-PolygonMap(map=colorado, id="cnty", data=county_simulated, variable="var3",legend_bar_length = unit(.5,"snpc"),legend_bar_width=unit(.02,"snpc"),legend_orientation = "vertical",
         map_colors = wpal("cool_earthtones"),legend_position = "right",  return_objects = T)$map

scary<-PolygonMap(map=colorado, id="cnty", data=county_simulated, variable="var3",legend_bar_length = unit(.5,"snpc"),legend_bar_width=unit(.02,"snpc"),legend_orientation = "vertical",
         map_colors = wpal("diverging_orange_purple_blue"), legend_position = "right", return_objects = T)$map

novalue<-PolygonMap(map=colorado, id="cnty", data=county_simulated, variable="var3",legend_bar_length= unit(.5,"snpc"),legend_bar_width=unit(.02,"snpc"),
         map_colors = wpal("sky"),  legend_position = "right",legend_orientation = "vertical", return_objects = T)$map
# print(calm, vp = vplayout(2, 1))
# print(scary, vp = vplayout(2, 2))
# print(novalue, vp = vplayout(2, 3))

pushViewport(viewport(layout = grid.layout(1, 3)))

print(calm, vp = vplayout(1, 1))
print(scary, vp = vplayout(1, 2))
print(novalue, vp = vplayout(1, 3))
```
...
grid.newpage()
pushViewport(viewport(layout = grid.layout(1, 2)))
print(quantile, vp = vplayout(1, 1))
print(pretty, vp = vplayout(1, 2))
```

To define the minimum and maximum of the color scale separately from the minimum and maximum of the data variable you are mapping, a two-item vector (with a minimum and maximum) can be passed to the map_colors_limits parameter. For those who wish to highlight or emphasize some portion of the data without using binned categories, the parameter map_color_breaks allows users to change how the color ramp is applied to the range of the data. Note that this approach does not change the values, or the "truth" presented by the maps-- it simply changes how the colors are stretched across the minimum and maximum of the scale. However, these changes can dramatically change the _appearance_ of the data.

```{r, echo=F, fig.height=3, fig.width=6, fig.align='center', message=F, warning=F, fig.cap="Changing how the colors are stretched across the data: (i) Evenly from the minimum to the maximum (top left), (ii) Artificially extending the range of the colors to 0-300, which is larger than the range of the data (top right), and (iii) Changing whether light or dark colors occupy more of the range of the data by shifting how the same colors are applied to the dataas in top left, or in a way where more dark or light colors cover the majority of the range (bottom)"}
default<-PolygonMap(map=colorado, 
                   id="cnty", 
                   data=county_simulated, 
                   variable="simulated",
                   legend_bar_width=unit(.02,"snpc"), 
                   map_subtitle="Default Settings",
                   legend_bar_length=unit(.4,"snpc"), font_size=8,
                   map_colors = rev(wpal("parrotfish")),
                   legend_position = "left",
                   return_objects = T,
                   include_titles = T)$map
limits<-PolygonMap(map=colorado, 
                   id="cnty", 
                   data=county_simulated, 
                   variable="simulated",
                   legend_bar_width=unit(.02,"snpc"), 
                   map_subtitle="Color Limits set c(0,300)",
                   legend_bar_length=unit(.4,"snpc"), font_size=8,
                   map_colors = rev(wpal("parrotfish")),
                   map_colors_limits = c(0,300),
                   legend_position = "right",
                   return_objects = T,
                   include_titles = T)$map
breaks1<-PolygonMap(map=colorado, 
                   id="cnty", 
                   data=colorado_data, 
                   variable="simulated",
                   map_subtitle="Color breaks c(0,.1,.9,1),",
                   legend_bar_width=unit(.02,"snpc"), 
                   legend_bar_length=unit(.4,"snpc"), font_size=8,
                   map_colors_breaks = c(0,.1,.9,1),
                   map_colors = rev(wpal("parrotfish")),
                   legend_position = "left",
                   return_objects = T,
                   include_titles = T)$map
breaks2<-PolygonMap(map=colorado, 
                   id="cnty", 
                   data=colorado_data, 
                   map_subtitle="Color breaks c(0,.5,.6,1)",
                   variable="simulated",
                   legend_bar_width=unit(.02,"snpc"), 
                   legend_bar_length=unit(.4,"snpc"), font_size=8,
                   map_colors_breaks = c(0,.5,.6,1),
                   map_colors = rev(wpal("parrotfish")),
                   legend_position = "right",
                   return_objects = T,
                   include_titles = T)$map
...
print(default, vp = vplayout(1, 1))
print(limits, vp = vplayout(1, 2))
print(breaks1, vp = vplayout(2, 1))
print(breaks2, vp = vplayout(2, 2))
```
...
```{r, echo=F, fig.height=1.5, fig.width=6, fig.align='center', message=F, warning=F, fig.cap="Using two different numeric values (20 and 50) to center the color scheme on a meaingful value"}
diverge_1<-PolygonMap(map=colorado, id="cnty", data=county_simulated,
                   map_subtitle="map_diverging_centervalue = 20",
                   variable="simulated",
                   map_diverging_centervalue = 20,
                   legend_bar_width=unit(.02,"snpc"),
                   legend_bar_length=unit(.4,"snpc"), font_size=8,
                   map_colors = wpal("diverging_tan_green_blue"),
                   legend_position = "left",
                   return_objects = T,
                   include_titles = T)$map
diverge_2<-PolygonMap(map=colorado, id="cnty", data=county_simulated,
                   map_subtitle="map_diverging_centervalue = 50",
                   map_diverging_centervalue = 50,
                   variable="simulated",
                   map_colors=wpal("diverging_tan_green_blue"),
                   legend_bar_width=unit(.02,"snpc"),
                   legend_bar_length=unit(.4,"snpc"), font_size=8,
...
grid.newpage()
pushViewport(viewport(layout = grid.layout(1, 2)))
print(diverge_1, vp = vplayout(1, 1))
print(diverge_2, vp = vplayout(1, 2))
...
```{r,fig.height=1, fig.width=4, fig.align='center', message=F, warning=F}
...
This functionality is integrated into the main mapping functions with the use of a 'histogram' parameter. When set to TRUE, the histogram will be placed at the bottom of the map, with the same limits and scaling as the map. To modify the histogram or its position, users can generate the histogram separately, using the function histogram_colorstats(), and modify the placements and formatting of the histogram as desired.

```{r,fig.height=4, fig.width=4, fig.align='center', message=F, warning=F, fig.cap="A map with a histogram of the data distribution"}
...
```{r, echo=F, fig.height=1.5, fig.width=6, fig.align='center', message=F, warning=F, fig.cap="Three plots with standardized color schemes output from visualizing a time series. For the purposes of this illustration, these plots have been aligned horizontally, but would usually appear in separate plots."}
results<-PolygonMap(map=colorado, id="cnty", data=county_simulated_time,
            variable="var3", series_dimension="year",series_sequence=c(2000,2005,2009),
            map_colors = wpal("berries"), legend_position = "right",legend_orientation = "vertical",
            map_subtitle="Year: ", map_title="Var 3",font_size=8,return_objects = T,include_titles=T)

grid.newpage()
pushViewport(viewport(layout = grid.layout(1, 3)))
print(results$map[['2000']], vp = vplayout(1, 1))
print(results$map[['2005']], vp = vplayout(1, 2))
print(results$map[['2009']], vp = vplayout(1, 3))
```
...
```{r, echo=T, fig.height=7, fig.width=5, fig.align='center', message=F, warning=F}
